...
class WideResNet(nn.Module):
    def __init__(self, depth=34, num_classes=10, widen_factor=10, dropRate=0.0, normalize=None):
        ...
        n = (depth - 4) / 6
        block = BasicBlock
        self.w_shape = (1, 3, 32, 32)
        Mu_ = torch.ones(self.w_shape).to('cuda:3')
        SD_ = torch.ones(self.w_shape).to('cuda:3')
        self.Mu_ = nn.Parameter(Mu_)
        self.SD_ = nn.Parameter(SD_)
        self.rand_weight = torch.normal(0, 1, self.w_shape).to('cuda:3')
    ...
    def set_rands(self):
        ...
        """for module in self.modules():
            try:
                module.set_rands(distribution)
            except:
                continue"""



def WideResNet32(num_classes=10, normalize=None):
